...
## Convenciones y guía de lectura

Este documento está organizado de forma **cronológica y por fases**, reflejando el orden real en el que se diseñó y construyó DevMind. Cada fase describe un incremento funcional o técnico completo y sigue, por lo general, la misma estructura: objetivo, diseño, implementación guiada por pruebas y conclusión.

Convenciones empleadas:

- **Fases y subfases.** El trabajo se numera como `Fase X` (incremento principal) y `Fase X.Y` (subfase). La hoja de ruta completa está en la sección [3. Fases del proyecto](#3-fases-del-proyecto).
- **Bloques de código.** Se usan para comandos, fragmentos de código, contratos HTTP, ejemplos de configuración y diagramas en texto (ASCII). El lenguaje del bloque (`ts`, `json`, `bash`, `sql`, `http`, `txt`, `gherkin`) indica su naturaleza.
- **Criterios de aceptación.** Se expresan en notación *Gherkin* (`Dado / Cuando / Entonces`), de forma que sean verificables y trazables a las pruebas automáticas.
- **Citas destacadas.** Los bloques citados (`>`) resaltan reglas de negocio, decisiones de diseño o ideas clave.
- **Metodología.** Salvo que se indique lo contrario, cada pieza se construye siguiendo **TDD**: primero la prueba (en rojo), después la implementación mínima (en verde) y, por último, el refactor.

Se recomienda leer primero la sección [2. Análisis y especificación de requisitos](#2-análisis-y-especificación-de-requisitos) para entender el *qué* y el *porqué*, y después recorrer las fases para ver el *cómo*.
...
| Término | Descripción |
|---------|-------------|
| **API** | *Application Programming Interface*. Interfaz que expone la funcionalidad del backend mediante endpoints HTTP. |
| **TFM** | Trabajo de Fin de Máster. |
| **RAG** | *Retrieval-Augmented Generation*. Genera respuestas con IA apoyándose en fragmentos de información recuperados (aquí, del código del proyecto). |
| **TDD** | *Test-Driven Development*. Desarrollo guiado por pruebas. |
| **Arquitectura limpia / hexagonal** | Estilo que separa dominio y aplicación de infraestructura y transporte mediante puertos (interfaces) y adaptadores (implementaciones). |
| **Puerto / Adaptador** | Un *puerto* es una interfaz definida por la aplicación; un *adaptador* es su implementación concreta en infraestructura. |
| **JWT** | *JSON Web Token*. Token firmado usado para autenticar peticiones. |
| **Embedding** | Representación vectorial del significado de un texto o fragmento de código, base de la búsqueda semántica. |
| **pgvector** | Extensión de PostgreSQL para almacenar y consultar vectores (*embeddings*). |
| **Chunk / CodeChunk** | Fragmento en el que se divide un archivo de código para indexarlo y recuperarlo de forma granular. |
| **Genkit** | Framework utilizado para integrar los modelos de IA (generación de *embeddings* y de respuestas). |
| **Modo invitado** | Modo de uso que permite probar la funcionalidad principal sin registro. |
| **Historia de usuario** | Requisito expresado desde la perspectiva del usuario (*rol – necesidad – beneficio*). |
| **Criterio de aceptación** | Condición verificable que determina si una historia de usuario se considera completada. |
| **MoSCoW** | Técnica de priorización: *Must / Should / Could / Won't*. |
| **ZIP** | Formato de compresión usado para subir el proyecto completo en una sola petición. |
...
- [Resumen](#resumen)
- [Convenciones y guía de lectura](#convenciones-y-guía-de-lectura)
- [Glosario de términos y acrónimos](#glosario-de-términos-y-acrónimos)
- [1. Objetivo del proyecto](#1-objetivo-del-proyecto)
- [2. Análisis y especificación de requisitos](#2-análisis-y-especificación-de-requisitos)
  - [2.1 Metodología de recogida de requisitos](#21-metodología-de-recogida-de-requisitos)
  - [2.2 Actores del sistema](#22-actores-del-sistema)
  - [2.3 Épicas](#23-épicas)
  - [2.4 Diagrama de casos de uso](#24-diagrama-de-casos-de-uso)
  - [2.5 Historias de usuario y criterios de aceptación](#25-historias-de-usuario-y-criterios-de-aceptación)
  - [2.6 Requisitos funcionales](#26-requisitos-funcionales)
  - [2.7 Requisitos no funcionales](#27-requisitos-no-funcionales)
  - [2.8 Estimación por puntos de historia](#28-estimación-por-puntos-de-historia)
  - [2.9 Priorización (MoSCoW) y trazabilidad a las fases](#29-priorización-moscow-y-trazabilidad-a-las-fases)
- [3. Fases del proyecto](#3-fases-del-proyecto)
- [Fase 0 — Inicialización del proyecto](#fase-0--inicialización-del-proyecto)
- [Fase 1 — Autenticación](#fase-1--autenticación)
  - [Fase 1.1 — Casos de uso de autenticación](#fase-11--casos-de-uso-de-autenticación)
  - [Fase 1.2 — Adaptadores reales de infraestructura](#fase-12--adaptadores-reales-de-infraestructura)
  - [Fase 1.3 — HTTP y endpoints de autenticación](#fase-13--http-y-endpoints-de-autenticación)
- [Fase 2 — Proyectos](#fase-2--proyectos)
  - [Fase 2.1 — Casos de uso de proyectos](#fase-21--casos-de-uso-de-proyectos)
  - [Fase 2.2 — Adaptadores de infraestructura](#fase-22--adaptadores-de-infraestructura)
  - [Fase 2.3 — HTTP y endpoints de proyectos](#fase-23--http-y-endpoints-de-proyectos)
- [Fase 3 — ProjectFiles](#fase-3--projectfiles)
  - [Fase 3.1 — Base interna de ProjectFiles](#fase-31--base-interna-de-projectfiles)
  - [Fase 3.2 — Adaptadores de infraestructura](#fase-32--adaptadores-de-infraestructura)
  - [Fase 3.3 — Endpoints HTTP para ProjectFiles](#fase-33--endpoints-http-para-projectfiles)
- [Fase 4 — PostgreSQL](#fase-4--postgresql)
  - [Fase 4.1 — Levantar PostgreSQL y conectar Node/Express](#fase-41--levantar-postgresql-y-conectar-nodeexpress)
  - [Fase 4.2 — Migraciones y tablas](#fase-42--migraciones-y-tablas)
  - [Fase 4.3 — Repositorios PostgreSQL](#fase-43--repositorios-postgresql)
- [Fase 5 — Subida de ZIP](#fase-5--subida-de-zip)
  - [Fase 5.1 — Caso de uso UploadProjectZipUseCase](#fase-51--caso-de-uso-uploadprojectzipusecase)
  - [Fase 5.2 — HTTP y endpoint de subida de ZIP](#fase-52--http-y-endpoint-de-subida-de-zip)
- [Fase 6 — Sincronización de resubidas de ZIP](#fase-6--sincronización-de-resubidas-de-zip)
  - [Fase 6.1 — Modificación de UploadProjectZipUseCase](#fase-61--modificación-de-uploadprojectzipusecase)
  - [Fase 6.1.1 — Tests unitarios de sincronización](#fase-611--tests-unitarios-de-sincronización)
  - [Fase 6.2 — Tests de integración HTTP y nueva respuesta](#fase-62--tests-de-integración-http-y-nueva-respuesta)
- [Fase 7 — CodeChunks](#fase-7--codechunks)
- [Fase 8 — Genkit + PostgreSQL/pgvector, embeddings y búsqueda semántica](#fase-8--genkit--postgresqlpgvector-embeddings-y-búsqueda-semántica)
  - [Fase 8.1 — Preparar PostgreSQL para pgvector](#fase-81--preparar-postgresql-para-pgvector)
  - [Fase 8.2 — Crear tabla para guardar embeddings](#fase-82--crear-tabla-para-guardar-embeddings)
  - [Fase 8.3 — Crear PostgresCodeChunkEmbeddingRepository con TDD](#fase-83--crear-postgrescodechunkembeddingrepository-con-tdd)
  - [Fase 8.4 — Caso de uso para generar embedding de un CodeChunk](#fase-84--caso-de-uso-para-generar-embedding-de-un-codechunk)
  - [Fase 8.5 — Crear el adaptador real GenkitEmbeddingGenerator](#fase-85--crear-el-adaptador-real-genkitembeddinggenerator)
  - [Fase 8.6 — Generar embeddings automáticamente cuando se generan CodeChunks](#fase-86--generar-embeddings-automáticamente-cuando-se-generan-codechunks)
  - [Fase 8.7 — Búsqueda semántica de chunks mediante embeddings](#fase-87--búsqueda-semántica-de-chunks-mediante-embeddings)
- [Fase 9 — IA/RAG: preguntas sobre el proyecto](#fase-9--iarag-preguntas-sobre-el-proyecto)
  - [Fase 9.1 — AskProjectQuestionUseCase](#fase-91--askprojectquestionusecase)
  - [Fase 9.2 — Endpoint HTTP](#fase-92--endpoint-http)
  - [Fase 9.3 — Implementación real del puerto AnswerGenerator con Genkit](#fase-93--implementación-real-del-puerto-answergenerator-con-genkit)
  - [Fase 9.4 — Filtrado de archivos binarios en la subida de ZIP](#fase-94--filtrado-de-archivos-binarios-en-la-subida-de-zip)
  - [Fase 9.5 — Detección del problema de cuota/límite al generar embeddings](#fase-95--detección-del-problema-de-cuotalímite-al-generar-embeddings)
- [Fase 10 — Indexación asíncrona y robusta](#fase-10--indexación-asíncrona-y-robusta)
  - [Fase 10.1 — Separar chunks de embeddings](#fase-101--separar-chunks-de-embeddings)
  - [Fase 10.2 — Tabla de estado de indexación](#fase-102--tabla-de-estado-de-indexación)
  - [Fase 10.3 — IndexProjectEmbeddingsUseCase](#fase-103--indexprojectembeddingsusecase)
  - [Fase 10.4 — Endpoint manual de indexación](#fase-104--endpoint-manual-de-indexación)
  - [Fase 10.5 — Endpoint para consultar estado de indexación](#fase-105--endpoint-para-consultar-estado-de-indexación)
  - [Fase 10.6 — Rate limit interno entre embeddings](#fase-106--rate-limit-interno-entre-embeddings)
  - [Fase 10.7 — Delay configurable desde .env](#fase-107--delay-configurable-desde-env)
  - [Fase 10.8 — Indexación automática en segundo plano](#fase-108--indexación-automática-en-segundo-plano)
- [Fase 11 — Refactor, endurecimiento y mejoras técnicas](#fase-11--refactor-endurecimiento-y-mejoras-técnicas)
  - [Fase 11.1 — Restricción explícita del algoritmo JWT](#fase-111--restricción-explícita-del-algoritmo-jwt)
  - [Fase 11.2 — Límite de subida de ZIP ampliado](#fase-112--límite-de-subida-de-zip-ampliado)
  - [Fase 11.3 — Refactor de fakes duplicados en los tests](#fase-113--refactor-de-fakes-duplicados-en-los-tests)
  - [Fase 11.4 — Corrección del código de estado HTTP en pregunta vacía](#fase-114--corrección-del-código-de-estado-http-en-pregunta-vacía)
  - [Fase 11.5 — Umbral de relevancia en el RAG](#fase-115--umbral-de-relevancia-en-el-rag)
  - [Fase 11.6 — Cabeceras de seguridad (helmet) y CORS configurable](#fase-116--cabeceras-de-seguridad-helmet-y-cors-configurable)
  - [Fase 11.7 — Rate limit en rutas caras (/ask y /upload)](#fase-117--rate-limit-en-rutas-caras-ask-y-upload)
  - [Fase 11.8 — Corrección de typos en comentarios](#fase-118--corrección-de-typos-en-comentarios)
  - [Fase 11.9 — Tests herméticos: generador de embeddings de test](#fase-119--tests-herméticos-generador-de-embeddings-de-test)
  - [Fase 11.10 — Sincronización del OpenAPI con la Fase 11](#fase-1110--sincronización-del-openapi-con-la-fase-11)
  - [Fase 11.11 — Indexación explícita: se elimina el scheduler automático de la subida del ZIP](#fase-1111--indexación-explícita-se-elimina-el-scheduler-automático-de-la-subida-del-zip)
  - [Fase 11.12 — Reintentos con backoff ante fallos transitorios del proveedor de embeddings](#fase-1112--reintentos-con-backoff-ante-fallos-transitorios-del-proveedor-de-embeddings)
  - [Fase 11.13 — Limpieza de `UploadProjectZipUseCase`: clasificador de dominio y puertos explícitos](#fase-1113--limpieza-de-uploadprojectzipusecase-clasificador-de-dominio-y-puertos-explícitos)
  - [Fase 11.14 — Protección de la ruta de indexación: guard de "ya en proceso" (409) y rate limit](#fase-1114--protección-de-la-ruta-de-indexación-guard-de-ya-en-proceso-409-y-rate-limit)
  - [Fase 11.15 — Consistencia de nombres: archivos, carpetas y una clase de error](#fase-1115--consistencia-de-nombres-archivos-carpetas-y-una-clase-de-error)
  - [Fase 11.16 — Cierre de huecos de cobertura: test del estado de indexación y test del rate limiting](#fase-1116--cierre-de-huecos-de-cobertura-test-del-estado-de-indexación-y-test-del-rate-limiting)
  - [Fase 11.17 — Eliminar la dependencia oculta de `GEMINI_API_KEY` en la suite de tests](#fase-1117--eliminar-la-dependencia-oculta-de-gemini_api_key-en-la-suite-de-tests)
  - [Fase 11.18 — Eliminar la creación manual de archivos: los archivos solo entran por ZIP](#fase-1118--eliminar-la-creación-manual-de-archivos-los-archivos-solo-entran-por-zip)
...
Antes de escribir una sola línea de código, DevMind se abordó como un producto software real: primero **entender el problema y a quién sirve**, y solo después decidir qué construir y en qué orden. Esta sección recoge ese trabajo previo de análisis, que es el que da origen a la hoja de ruta de fases del apartado siguiente.

El resultado no es una lista cerrada de "todo lo que hará la aplicación", sino una **especificación viva**: un conjunto de historias de usuario priorizadas, con criterios de aceptación verificables, que se ha ido refinando conforme el proyecto avanzaba.

### 2.1 Metodología de recogida de requisitos

Se ha seguido un enfoque **ágil y centrado en el usuario**, apoyado en técnicas ampliamente usadas en la industria:

- **Historias de usuario** como unidad básica de requisito, redactadas con la plantilla *rol – necesidad – beneficio*:

  > Como **\<rol\>**, quiero **\<funcionalidad\>** para **\<beneficio / valor\>**.

- **Criterios de aceptación en formato Gherkin** (*Dado / Cuando / Entonces*), de forma que cada historia sea **verificable** y traducible directamente a pruebas automáticas. Esto encaja de forma natural con la metodología **TDD** que se sigue en toda la implementación: los criterios de aceptación son, en la práctica, la especificación de los tests.

- **Criterios INVEST** para validar que cada historia sea *Independiente, Negociable, Valiosa, Estimable, Pequeña y Testeable*.

- **Priorización MoSCoW** (*Must / Should / Could / Won't*) para separar lo imprescindible del primer producto usable de lo deseable o futuro.

- **Story Mapping**: las historias se agrupan en **épicas** (grandes capacidades del sistema) y se ordenan en un *backlog* que después se traduce en la secuencia de fases de desarrollo.

El flujo conceptual fue:

```txt
Problema real
      ↓
Identificación de actores y objetivos
      ↓
Épicas (grandes capacidades)
      ↓
Historias de usuario + criterios de aceptación
      ↓
Priorización (MoSCoW) y trazabilidad a fases
      ↓
Implementación guiada por TDD
```
...
|-------|-------------|----------------------|
| **Invitado** | Persona que usa DevMind **sin registrarse**. Puede probar el análisis de un proyecto de forma efímera. | Evaluar la herramienta sin fricción, antes de crear una cuenta. |
| **Usuario registrado** | Persona autenticada mediante email y contraseña. | Crear y **persistir** sus proyectos, archivos, indexaciones e historial de conversaciones. |
| **Sistema de IA (RAG)** | Componente que indexa el código y responde preguntas en lenguaje natural apoyándose en el contenido del proyecto. | Convertir el código en conocimiento consultable. |
| **Sistema (backend)** | La propia API, que aplica reglas de seguridad, propiedad de datos y validaciones. | Garantizar integridad, aislamiento entre usuarios y consistencia de datos. |

### 2.3 Épicas

Las capacidades del sistema se organizaron en las siguientes épicas, que son las que estructuran el *backlog* y, más adelante, las fases:

```txt
EP-01  Identidad y control de acceso          (registro, login, usuario actual)
EP-02  Gestión de proyectos                   (CRUD de proyectos por usuario)
EP-03  Gestión de archivos de código          (CRUD de ficheros + subida de ZIP + resincronización)
EP-04  Indexación y búsqueda semántica         (chunks, embeddings, pgvector, indexación asíncrona)
EP-05  Consultas inteligentes (RAG)           (preguntas en lenguaje natural sobre el proyecto)
EP-06  Historial de conversaciones            (persistencia de preguntas y respuestas por proyecto)
EP-07  Modo invitado                          (uso de la herramienta sin registro)
EP-08  Producto final (futuro)                (frontend, mejoras de RAG y despliegue)
```

### 2.4 Diagrama de casos de uso

El siguiente diagrama resume, de un vistazo, qué actores interactúan con DevMind y qué casos de uso puede ejecutar cada uno. Cada caso de uso (`UCx`) se corresponde con una o varias historias de usuario del apartado siguiente.

```txt
                    ┌──────────────────────── Sistema DevMind ────────────────────────┐
                    │                                                                 │
                    │   (UC1)  Registrarse             (UC7)  Resincronizar ZIP       │
                    │   (UC2)  Iniciar sesión          (UC8)  Indexar proyecto        │
                    │   (UC3)  Ver usuario actual      (UC9)  Buscar semánticamente   │
   ┌──────────┐     │   (UC4)  Gestionar proyectos     (UC10) Preguntar al proyecto   │
   │ Invitado │─────┤   (UC5)  Gestionar archivos             (RAG)                   │
   └──────────┘     │   (UC6)  Subir proyecto (ZIP)    (UC11) Consultar historial     │
                    │                                                                 │
   ┌──────────────┐ │                                                                 │
   │   Usuario    │─┤                                                                 │
   │  registrado  │ │                                                                 │
...
Las asociaciones entre actores y casos de uso, así como las relaciones `«include»` / `«extend»`, son las siguientes:

```txt
Asociaciones actor → caso de uso
────────────────────────────────
Invitado             →  UC1, UC2  y, en modo invitado, UC6 y UC10
Usuario registrado   →  UC2, UC3, UC4, UC5, UC6, UC7, UC9, UC10, UC11
Sistema de IA (RAG)  →  UC8, UC9, UC10   (genera embeddings y respuestas)

Relaciones «include» / «extend»
───────────────────────────────
UC10 Preguntar al proyecto  ──«include»──▶  UC9  Buscar semánticamente
UC9  Buscar semánticamente  ──«include»──▶  UC8  Indexar proyecto (requisito previo)
UC6  Subir proyecto (ZIP)   ──«extend»───▶  UC8  Indexar proyecto (dispara la indexación)
```

### 2.5 Historias de usuario y criterios de aceptación

A continuación se detallan las historias de usuario más relevantes de cada épica junto con sus criterios de aceptación. Cada historia tiene un identificador (`HU-XX`) que se usa después en la matriz de trazabilidad.

---

#### EP-01 · Identidad y control de acceso

**HU-01 — Registro de usuario**

> Como **visitante**, quiero **registrarme con mi email y una contraseña** para **disponer de una cuenta donde guardar mis proyectos**.

```gherkin
Dado un email que no está registrado y una contraseña válida
Cuando envío una petición de registro
Entonces se crea mi usuario y recibo una respuesta 201 con mis datos públicos

Dado un email que ya existe en el sistema
Cuando intento registrarme con ese email
Entonces recibo un error 409 y no se crea ningún usuario duplicado
...
> Como **usuario registrado**, quiero **iniciar sesión con mis credenciales** para **obtener un token que me identifique en el resto de la API**.

```gherkin
Dado un email y una contraseña correctos
Cuando envío una petición de login
Entonces recibo un token JWT válido

Dado un email o contraseña incorrectos
Cuando intento iniciar sesión
Entonces recibo un error 401 de credenciales inválidas
```

**HU-03 — Consultar mi usuario**

> Como **usuario autenticado**, quiero **consultar los datos de mi usuario actual** para **confirmar mi identidad dentro de la aplicación**.

```gherkin
Dado un token JWT válido
Cuando pido mis datos de usuario
Entonces recibo mis datos públicos (sin la contraseña)

Dado que no envío token o el token es inválido
Cuando pido mis datos de usuario
...
> Como **usuario autenticado**, quiero **crear un proyecto** para **agrupar bajo él el código que quiero analizar**.

```gherkin
Dado un token válido y un nombre de proyecto
Cuando creo un proyecto
Entonces se crea asociado a mi usuario y recibo 201

Regla de seguridad
El propietario del proyecto se toma siempre del token, nunca del body,
para que nadie pueda crear proyectos a nombre de otro usuario
```

**HU-05 — Listar mis proyectos**

> Como **usuario autenticado**, quiero **listar mis proyectos** para **ver únicamente los que me pertenecen**.

```gherkin
Dado que existen proyectos de varios usuarios
Cuando pido mi lista de proyectos
Entonces solo veo los proyectos cuyo propietario soy yo
```

**HU-06 / HU-07 — Ver y borrar un proyecto concreto**

> Como **usuario autenticado**, quiero **ver o borrar un proyecto concreto** para **gestionar solo lo que es mío**.

```gherkin
Dado un proyecto que me pertenece
Cuando lo consulto o lo borro
Entonces la operación tiene éxito (200 / 204)

Dado un proyecto que pertenece a otro usuario
Cuando intento consultarlo o borrarlo
Entonces recibo un 404, sin revelar siquiera que ese proyecto existe
```

---

#### EP-03 · Gestión de archivos de código

**HU-08 — Gestionar archivos de un proyecto**

> Como **usuario autenticado**, quiero **crear, listar, consultar y borrar archivos dentro de mi proyecto** para **mantener su contenido de código**.

```gherkin
Dado un proyecto que me pertenece
Cuando gestiono sus archivos
Entonces se aplican las mismas reglas de propiedad que en el proyecto
Y a cada archivo se le calcula su tamaño y un hash de contenido
```

**HU-09 — Subir un proyecto como ZIP**

> Como **usuario autenticado**, quiero **subir un proyecto comprimido en ZIP** para **no tener que crear los archivos uno a uno**.

```gherkin
Dado un ZIP con el código de mi proyecto
Cuando lo subo al endpoint de subida
Entonces se extraen sus archivos y se guardan como archivos del proyecto
Y se ignoran carpetas irrelevantes como node_modules, .git o dist

Dado un ZIP subido a un proyecto que no es mío
Cuando intento la subida
Entonces recibo un 404
```

**HU-10 — Resincronizar tras volver a subir un ZIP**

> Como **usuario autenticado**, quiero **volver a subir el ZIP cuando cambie mi código** para **que el proyecto refleje el estado actual sin duplicados**.

```gherkin
Dado un proyecto ya subido previamente
Cuando vuelvo a subir el ZIP con cambios
Entonces los archivos nuevos se añaden
Y los modificados se actualizan (cambia su hash)
Y los que ya no existen se eliminan
```

---

#### EP-04 · Indexación y búsqueda semántica

**HU-11 — Búsqueda semántica sobre el código**

> Como **usuario autenticado**, quiero **que el código de mi proyecto se indexe** para **poder buscar por significado y no solo por texto exacto**.

```gherkin
Dado un proyecto con archivos subidos
Cuando se indexa el proyecto
Entonces su código se divide en fragmentos (chunks)
Y a cada fragmento se le calcula un embedding almacenado en pgvector

Dado un proyecto ya indexado
Cuando realizo una búsqueda semántica
Entonces recibo los fragmentos más parecidos al significado de mi consulta
```

**HU-12 — Indexación asíncrona y robusta**

> Como **usuario autenticado**, quiero **que la indexación no bloquee la subida** para **poder seguir trabajando mientras el proyecto se procesa**.

```gherkin
Dado que subo un proyecto grande
Cuando comienza la indexación
Entonces se ejecuta de forma asíncrona y puedo consultar su estado
Y si un fragmento falla, el proceso es resiliente y no corrompe el resto
```

---

#### EP-05 · Consultas inteligentes (RAG)

**HU-13 — Preguntar al proyecto en lenguaje natural**

> Como **usuario autenticado**, quiero **hacer preguntas en lenguaje natural sobre mi proyecto** para **entenderlo sin tener que leer todo el código**.

```gherkin
Dado un proyecto indexado
Cuando hago una pregunta como "¿qué endpoints existen?"
Entonces recibo una respuesta generada por IA
Y la respuesta se apoya en fragmentos reales del código del proyecto (sources)

Dado un proyecto que no es mío
Cuando intento hacerle preguntas
Entonces recibo un 404
```

---

#### EP-06 · Historial de conversaciones

**HU-14 — Guardar y consultar el historial**

> Como **usuario autenticado**, quiero **que mis preguntas y respuestas se guarden por proyecto** para **poder retomar conversaciones anteriores**.

```gherkin
Dado que hago una pregunta sobre un proyecto
Cuando recibo la respuesta
Entonces la pareja pregunta/respuesta se persiste asociada a ese proyecto y a mí

Dado un proyecto con conversaciones previas
Cuando consulto su historial
Entonces veo las preguntas y respuestas anteriores en orden
```
...
**HU-15 — Probar DevMind sin registrarse**

> Como **visitante**, quiero **probar el análisis de un proyecto sin registrarme** para **evaluar la herramienta antes de crear una cuenta**.

```gherkin
Dado que no tengo cuenta
Cuando uso el modo invitado
Entonces puedo subir un proyecto y hacerle preguntas
Sin necesidad de credenciales
```

### 2.6 Requisitos funcionales

Las historias anteriores se consolidan en los siguientes requisitos funcionales (RF):

| ID | Requisito funcional | Épica |
|----|---------------------|-------|
| RF-01 | El sistema permite registrar usuarios con email y contraseña. | EP-01 |
| RF-02 | El sistema autentica usuarios y emite un token JWT. | EP-01 |
| RF-03 | El sistema permite consultar los datos del usuario autenticado. | EP-01 |
| RF-04 | El sistema permite crear, listar, consultar y borrar proyectos por usuario. | EP-02 |
| RF-05 | El sistema garantiza que un usuario solo accede a sus propios proyectos. | EP-02 |
| RF-06 | El sistema permite gestionar archivos de código dentro de un proyecto. | EP-03 |
| RF-07 | El sistema permite subir un proyecto comprimido en ZIP e ignorar carpetas irrelevantes. | EP-03 |
| RF-08 | El sistema resincroniza los archivos al volver a subir un ZIP (altas, cambios y bajas). | EP-03 |
| RF-09 | El sistema fragmenta el código y genera embeddings para búsqueda semántica. | EP-04 |
| RF-10 | El sistema ejecuta la indexación de forma asíncrona y resiliente. | EP-04 |
| RF-11 | El sistema responde preguntas en lenguaje natural apoyándose en el código (RAG). | EP-05 |
| RF-12 | El sistema persiste el historial de preguntas y respuestas por proyecto. | EP-06 |
| RF-13 | El sistema permite usar la funcionalidad principal en modo invitado, sin registro. | EP-07 |

### 2.7 Requisitos no funcionales

| ID | Requisito no funcional | Descripción |
|----|------------------------|-------------|
| RNF-01 | **Seguridad** | Contraseñas hasheadas, autenticación por JWT y aislamiento estricto de datos entre usuarios (un usuario nunca ve datos de otro). |
| RNF-02 | **Arquitectura** | Arquitectura limpia / hexagonal: dominio, aplicación, infraestructura y transporte desacoplados mediante puertos y adaptadores. |
| RNF-03 | **Calidad y mantenibilidad** | Desarrollo guiado por pruebas (TDD), con tests unitarios, de integración y E2E que respaldan cada requisito funcional. |
| RNF-04 | **Persistencia** | Datos almacenados en PostgreSQL, con `pgvector` para los embeddings y borrado en cascada para evitar datos huérfanos. |
| RNF-05 | **Rendimiento y escalabilidad** | La indexación de proyectos grandes no bloquea las peticiones del usuario (procesamiento asíncrono). |
| RNF-06 | **Trazabilidad de errores** | Errores de dominio propios traducidos de forma consistente a códigos HTTP mediante un middleware central. |
| RNF-07 | **Usabilidad / adopción** | Modo invitado que reduce la fricción de entrada permitiendo probar la herramienta sin registro. |

### 2.8 Estimación por puntos de historia

Para dimensionar el esfuerzo se utilizó **estimación relativa por puntos de historia** (*story points*) mediante *planning poker*, empleando la escala de **Fibonacci** (1, 2, 3, 5, 8, 13…). Un punto de historia no mide horas, sino una combinación de **complejidad técnica, volumen de trabajo e incertidumbre**. Como historia de referencia se tomó **HU-03** (ver usuario actual), a la que se asignaron **2 puntos**; el resto se estimó por comparación con ella.

| Historia | Resumen | Factores de complejidad | Puntos |
|----------|---------|-------------------------|:------:|
| HU-01 | Registro de usuario | Hasheo de contraseña y control de duplicados | 3 |
| HU-02 | Inicio de sesión | Verificación de credenciales y emisión de JWT | 3 |
| HU-03 | Ver usuario actual | Lectura simple protegida (historia de referencia) | 2 |
| HU-04 | Crear proyecto | CRUD sencillo; `ownerId` derivado del token | 2 |
| HU-05 | Listar proyectos | Filtrado por propietario | 2 |
| HU-06 | Ver proyecto | Lectura con control de propiedad | 2 |
| HU-07 | Borrar proyecto | Borrado con control de propiedad | 2 |
| HU-08 | Gestionar archivos | CRUD anidado; cálculo de tamaño y hash | 5 |
| HU-09 | Subir proyecto en ZIP | Multipart, extracción y filtrado de carpetas | 8 |
| HU-10 | Resincronizar ZIP | Diff de altas/cambios/bajas por hash | 8 |
| HU-11 | Búsqueda semántica | Chunking, embeddings, pgvector y similitud | 13 |
| HU-12 | Indexación asíncrona | Procesamiento en segundo plano, estado y resiliencia | 8 |
| HU-13 | Preguntar al proyecto (RAG) | Orquestación RAG, prompt y recuperación de contexto | 13 |
| HU-14 | Historial de conversaciones | Persistencia y consulta de preguntas/respuestas | 5 |
| HU-15 | Modo invitado | Flujo alternativo sin autenticación (sesión efímera) | 8 |

Agrupando por épica se obtiene el tamaño total del *backlog* implementado:

| Épica | Puntos |
|-------|:------:|
| EP-01 Identidad y acceso | 8 |
| EP-02 Gestión de proyectos | 8 |
| EP-03 Archivos y ZIP | 21 |
| EP-04 Indexación y búsqueda | 21 |
| EP-05 Consultas RAG | 13 |
| EP-06 Historial | 5 |
| EP-07 Modo invitado | 8 |
| **Total (backlog implementado)** | **84** |

> La épica **EP-08** (frontend, mejoras de RAG y despliegue) queda fuera del alcance del MVP de backend y, por tanto, pendiente de estimación en detalle.

### 2.9 Priorización (MoSCoW) y trazabilidad a las fases

Por último, las épicas se priorizaron con MoSCoW y se ordenaron en el *backlog*, que es exactamente lo que da lugar a la secuencia de fases descrita en el apartado siguiente. Esta matriz sirve de **trazabilidad** entre lo que se pidió (requisitos) y lo que se construyó (fases).

| Épica | Historias | Prioridad MoSCoW | Fase(s) de implementación |
|-------|-----------|------------------|---------------------------|
| EP-01 Identidad y acceso | HU-01 … HU-03 | Must | Fase 1 |
| EP-02 Gestión de proyectos | HU-04 … HU-07 | Must | Fase 2 |
| EP-03 Archivos y ZIP | HU-08 … HU-10 | Must | Fases 3, 5 y 6 |
| Persistencia real (soporte) | — | Must | Fase 4 |
| EP-04 Indexación y búsqueda | HU-11, HU-12 | Must | Fases 7, 8 y 10 |
| EP-05 Consultas RAG | HU-13 | Must | Fase 9 |
| Endurecimiento técnico (soporte) | — | Should | Fase 11 |
| EP-06 Historial | HU-14 | Should | Fase 12 |
| EP-07 Modo invitado | HU-15 | Should | Fase 13 |
| EP-08 Producto final | — | Could | Fases 14, 15 y 16 (pendientes) |

Con los requisitos claros y priorizados, el proyecto se descompuso en la siguiente hoja de ruta de fases.
